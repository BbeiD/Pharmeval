// ===================== BACKEND DE DEMONSTRATION / TEST (Sprint 21) =====================
// CORRECTIF (suite au signalement "boucle infinie de controle d'acces") :
// ce fichier vivait auparavant sous tests/fake-firestore-backend.mjs et
// etait importe DEPUIS admin/catalog-sync-wiring.js - un fichier de
// PRODUCTION qui dependait donc d'un fichier de TEST potentiellement non
// deploye sur l'hebergement reel. Si ce fichier etait absent, l'ensemble
// du module catalog-sync.js echouait au chargement, et #cs-loading
// restait affiche indefiniment (aucun code n'atteignait jamais
// initAuthGate()). Deplace ici, dans js/services/, aux cotes des autres
// fichiers reellement deployes.
//
// Implemente EXACTEMENT le meme contrat de fonctions que la vraie
// integration production decrite dans NOTES_INTEGRATION_PRODUCTION.md -
// reutilise les memes conventions (find-or-create par cle normalisee,
// path/pathLabels pour les sections, statut "draft" pour toute creation).
// Utilise par catalog-sync-wiring.js (mode demonstration, voir le bandeau
// visible sur admin/catalog-sync.html) ET par les tests
// (tests/test-sync-engine-e2e.mjs, tests/test-catalog-sync-workflow.mjs).

import { normalizeForDedup, findPotentialDuplicates } from './normalization-utils.js';
...
export class FakeFirestoreBackend {
  ...
  resolveCompetency = async ({ label, dryRun, cache }) => {
    ...
    if (!competencyId) {
      ...
      const existingEntries = Array.from(this.competencies.values()).map(function(c) { return { key: normalizeForDedup(c.name), label: c.name }; });
      potentialDuplicates = findPotentialDuplicates(label, existingEntries);
      if (!dryRun) { this.competencies.set(competencyId, { id: competencyId, name: label }); this.competenciesByKey.set(key, competencyId); }
    }
    const result = { competencyId: competencyId, action: action, label: label, key: key, potentialDuplicates: potentialDuplicates };
