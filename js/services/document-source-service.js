// ===================== SERVICE D'ORCHESTRATION DES SOURCES DOCUMENTAIRES (Sprint 20 / Correctif 20.2) =====================
...
//   - js/services/authorization-service.js              (MANAGE_GLOBAL_CATALOG -
//     voir CORRECTIF ci-dessous)
//   - js/services/audit-service.js                      (REUTILISE audit_logs, Sprint 8)
//
// CORRECTIF (Sprint 20.2, "Catalogue documentaire global") : la source
// documentaire est desormais une entite GLOBALE de la plateforme (plus
// aucun `organizationId`) - la permission requise passe de
// MANAGE_QUESTIONS a MANAGE_GLOBAL_CATALOG, une permission DISTINCTE
// destinee a un "administrateur du catalogue global", different d'un
// futur "administrateur d'organisation" qui ne devra jamais pouvoir
// modifier ce catalogue automatiquement (cadrage, "Administration et
// permissions"). Voir RAPPORT_CORRECTIF_SPRINT20_2.md.
//
 ...
 * (type, statut).
 * @param {{sourceType?:string, status?:string}} [options]
...
/**
 * Édite les champs éditables d'une source (jamais id/sourceType/status -
 * voir changeDocumentSourceStatus() pour les transitions de statut,
 * séparées).
 * @param {object} source
 * @param {object} fields
 * @returns {Promise<object>}
...
export async function editDocumentSource(source, fields) {
  ...
  const editable = ['name', 'shortCode', 'sourceOrganizationName', 'version', 'academicYear', 'language', 'description', 'metadata', 'display'];
  const payload = {};
