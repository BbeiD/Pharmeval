...
export async function submitCreateParcours() {
  ...
  if (result.status === 'success') {
    ...
    ['parcours-create-name', 'parcours-create-description', 'parcours-create-audience', 'parcours-create-color', 'parcours-create-icon'].forEach(function(id) {
      const el = document.getElementById(id);
      if (el) el.value = '';
    });
    await loadPage();
...
function detailHtml(p) {
  ...
  if (p.color) html += '<span class="bank-chip" style="background:' + escapeHtml(p.color) + ';color:#fff;">' + escapeHtml(p.color) + '</span>';
  html += '</div>';
  ...
  html += '<div class="btn-row"><input type="text" id="parcours-new-competency-name" class="bank-select" placeholder="Nom de la nouvelle compétence"><button class="btn-primary" onclick="submitAddCompetency()">Ajouter</button></div>';
  html += '</div>';
  ...
  html += '<input type="text" id="parcours-edit-color" class="bank-select" value="' + escapeHtml(p.color || '') + '">';
...
async function renderTimeline(p) {
  ...
  if (result.error) { container.textContent = 'Impossible de charger l\u2019historique pour le moment.'; return; }
